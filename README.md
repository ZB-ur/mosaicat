...
- **崩溃恢复** — `mosaicat resume` 从断点续跑；`--from <stage>` 可指定从某个 stage 重跑（自动清理该 stage 及下游的 artifact）
...
- **Stage Resume** 每个 stage 完成后持久化状态，崩溃后 `mosaicat resume` 从断点续跑；`--from <stage>` 支持指定 stage 重跑并自动清理下游 artifact
