...
<p align="center">
  <strong>Spec Coding — 规约驱动的 AI 流水线：一条指令产出<br/>分层规约和经过验证的代码。</strong>
</p>
...
- **验证即规约一致性校验** — 4 项纯程序化检查（零 LLM，完全确定性）+ 4 项 LLM 分析限定在结构化 manifest（~1–2 KB）内，取代 50k token 的全量审查
- **规约知识跨运行积累** — Prompt 进化和 Skill 捕获让每次交付成为组织规约知识 — 人类审批是安全门控
...
- **多 LLM 支持** — Claude、OpenAI、Gemini、DeepSeek、通义千问、豆包、Kimi、MiniMax；运行 `mosaicat setup` 一键切换，重复执行可随时更换供应商
...
每个 Agent 生成一份 **manifest**（~1–2 KB），声明结构性事实：覆盖了哪些 Feature ID、生成了哪些文件。Validator 执行 **8 项分层检查** — 4 项纯程序化（集合交叉、文件存在性，零 LLM）+ 4 项 LLM 辅助分析（限定在 manifest 范围内）。这是规模化规约一致性校验：验证每层规约与其他层的结构一致性，而非依赖另一个 AI 做质量判断。
...
每个 Agent 生成 ~1–2 KB 的 manifest 声明结构性事实。Validator 执行 8 项分层检查 — 4 项纯程序化（零 LLM）+ 4 项 LLM 辅助（限定 manifest 范围）。在企业级流水线中，你不能依赖概率性的质量门控。
