...
- [ ] Orchestrator 用迭代循环替代递归 `executeStage`，消除栈溢出风险
- [ ] Orchestrator 的 Tester→Coder 修复循环从索引操作改为独立方法
- [ ] Artifact 层从全局可变状态改为 `ArtifactStore` 实例，按 run 隔离
- [ ] Coder Agent 从 1312 行单文件拆分为 Planner / Builder / BuildVerifier / SmokeRunner
- [ ] Evolution Engine 消灭 9 个 silent catch，引入统一错误处理（log + fallback）
- [ ] Validator 消灭 7 个 silent catch，对损坏的 manifest 返回显式 "unreadable" 状态
- [ ] Context Manager 在 prompt 文件缺失时 fail-fast 或 log warning，不静默降级
- [ ] Retrying Provider 设有限重试上限（默认 20 次）+ 总耗时熔断器
- [ ] 添加优雅关闭处理（SIGINT/SIGTERM → 完成当前 stage 写入后退出）
- [ ] 补齐 resume 流程集成测试（覆盖 `resumeRun()`、`--from` stage reset、artifact cleanup）
- [ ] 补齐 Coder shell 命令执行路径测试（setup/build/verify/smoke-test）
- [ ] 统一 console.log 到 logger 模块（消除 30+ 处绕过 logger 的直接输出）
- [ ] Orchestrator 可变 config 注入改为 clone-before-mutate 模式
- [ ] SecurityAuditor 排除 .env 文件内容扫描，只检查存在性
...
| ArtifactStore 实例化替代全局变量 | 消除并发风险和测试隔离问题，为未来并行 stage 铺路 | — Pending |
| Coder 拆分为 4 个子模块 | 1312 行 6 个职责的单文件无法单独测试和维护 | — Pending |
